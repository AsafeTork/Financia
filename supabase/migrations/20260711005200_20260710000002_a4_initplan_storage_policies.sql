-- A4: Envolver (SELECT auth.uid()) em (SELECT auth.uid()) nas 4 policies storage.objects (initPlan)
-- Otimização de performance: evita chamada de (SELECT auth.uid()) por linha (por-row evaluation)
...
create policy logos_authenticated_select on storage.objects
  ...
  using (
    bucket_id = 'logos'
    and (
      (storage.foldername(name))[1] = (select (SELECT auth.uid()))::text
      or exists (select 1 from public.user_roles where user_id = (select (SELECT auth.uid())) and role = 'admin')
    )
  );
