...
--
-- Achado 🔴: o gate `(select role from user_roles where user_id=(SELECT auth.uid())) <> 'admin'`
...
as $function$
begin
  if not exists (select 1 from user_roles where user_id = (SELECT auth.uid()) and role = 'admin') then
