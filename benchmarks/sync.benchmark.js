...
function setupMockSupabase() {
  const createMockQb = () => {
    const qb = {
      ...
      gte: function() { return qb; },
      order: function() { return qb; },
      limit: function() { return qb; },
...
async function runBenchmark() {
  console.log('Starting syncAll 10k benchmark...');
  ...
  const testData = generateTestData(1000); // 3000 total records
  console.log('Generated test data, seeding database...');
  await seedDatabase(testData);
  console.log('Database seeded, running syncAll...');
  ...
  const recordsPerSecond = (3000 / totalTimeMs) * 1000;
  ...
  const metrics = {
    ...
    success: result,
    scaledTo10k: Math.round(totalTimeMs * 3.333), // extrapolate to 10k
  };
  ...
  if (!result) {
    console.error('ERROR: syncAll returned false');
    process.exit(1);
  }
