---
name: gsap
description: Official GSAP (GreenSock Animation Platform) skill — core API, timelines, ScrollTrigger, plugins (Flip, Draggable, SplitText, etc.), React integration, performance, and framework patterns. Use for any GSAP animation work across vanilla JS, React, Vue, Svelte.
---
...
Sequencing multiple tweens with precise control.
...
tl.to(".box1", { x: 100 })
  .to(".box2", { y: 100 }, "-=0.2")  // overlap by 0.2s
  .to(".box3", { rotation: 360 }, "<")  // start with previous
  .addLabel("midpoint")
  .to(".box4", { scale: 2 }, "midpoint");
...
tl.to(".b", { x: 100 }, "+=1")    // 1s after previous ends
tl.to(".c", { x: 100 }, "-=0.5")  // 0.5s before previous ends (overlap)
tl.to(".d", { x: 100 }, "<")      // at same time as previous start
tl.to(".e", { x: 100 }, ">")      // at same time as previous end
tl.to(".f", { x: 100 }, "label")  // at label
tl.to(".g", { x: 100 }, "label+=0.5")
```
...
gsap.to(".panel", {
  scrollTrigger: {
    trigger: ".panel",
    start: "top top",
    end: "+=500",  // scroll 500px while pinned
    pin: true,
    pinSpacing: true,  // adds padding to keep flow
    scrub: 1,
  }
});
```
...
```javascript
gsap.registerPlugin(ScrollTrigger, Flip, Draggable, SplitText, ScrollToPlugin);
```
...
// DOM changes happen here (reorder, filter, etc.)
Flip.from(state, {
  duration: 0.5,
  ease: "power2.inOut",
  stagger: 0.05,
...
gsap.utils.snap(10, value);  // snap to nearest 10
gsap.utils.wrap(0, 10, value);
gsap.utils.toArray(".items");  // NodeList to Array
gsap.utils.selector(container);  // scoped selector
gsap.utils.pipe(fn1, fn2, fn3);  // function composition
```
...
export function Hero() {
  useGSAP(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from(".subtitle", { y: 30, opacity: 0, duration: 0.6, delay: 0.2 });
...
- ✅ Animate **transforms** (`x`, `y`, `scale`, `rotation`) and `opacity` only
- ✅ Use `will-change: transform` sparingly on animating elements
- ✅ Batch DOM reads/writes with `gsap.batch()`
- ✅ Use `ScrollTrigger.normalizeScroll()` for smooth scroll
- ✅ Kill tweens on unmount: `ctx.revert()` or `tween.kill()`
- ❌ Avoid animating `width`, `height`, `top`, `left`, `margin`, `padding`
- ❌ Avoid `window.addEventListener("scroll", ...)` — use ScrollTrigger
- ❌ Avoid `requestAnimationFrame` loops touching React state — use `useGSAP` / `gsap.context()`
...
| Task | Code |
|------|------|
| Basic tween | `gsap.to(".el", { x: 100, duration: 1 })` |
| From state | `gsap.from(".el", { opacity: 0, y: 20 })` |
| Timeline | `gsap.timeline().to(...).to(...)` |
| Stagger | `gsap.to(".items", { y: 20, stagger: 0.1 })` |
| ScrollTrigger | `scrollTrigger: { trigger: ".el", scrub: 1 }` |
| Pin | `pin: true, start: "top top", end: "+=500"` |
| React cleanup | `useGSAP(() => { ... }, [])` |
| Reduced motion | `mm.add({ reduceMotion: "(prefers-reduced-motion: reduce)" }, ...)` |
| Flip | `Flip.from(state, { duration: 0.5 })` |
...
# For plugins (trial/free)
npm install gsap@npm:@gsap/shockingly  # Club GSAP for premium plugins
```
...
- `gsap-plugins` — Flip, Draggable, SplitText, etc.
- `gsap-react` — React integration patterns
- `gsap-utils` — Utility functions
- `gsap-performance` — Performance optimization
- `motion-principles` — Timing, easing, craft fundamentals