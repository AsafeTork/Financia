...
- **Correções administrativas e visuais em andamento (2026-08-10):** corrigidos os contratos de plano/preço customizado para o schema atual (`p_*` e `custom_prices` JSONB), criada Edge Function autenticada para atualização de clientes, corrigida exclusão administrativa com escopo de usuário, ampliadas as barras do gráfico comparativo da landing, favicon trocado por símbolo legível em tamanhos pequenos, tokens aplicados ao preview do editor e removido handler duplicado de navigation preload. Validação local rápida passou; validação completa foi interrompida a pedido do responsável e deverá ocorrer somente no workflow CI.
...
- **Preparação de lançamento e descoberta** (2026-08-10): corrigidos sitemap/robots para `financiabr.me`, metadados e JSON-LD de preços/recursos, favicon SVG de alto contraste e `public/llms.txt`; gráfico da landing agora mostra entradas/saídas lado a lado; CTA final ganhou contraste; mapeamento Tailwind corrigido de HSL inválido para tokens OKLCH, eliminando contornos/fundos incorretos em temas e admin; templates de e-mail não sugerem envio de senha e usam domínio oficial. Relatório em `docs/Growth/MARKET_VALIDATION_EXECUTION_REPORT_2026-08-10.md`. Mercado e receita continuam não validados.
