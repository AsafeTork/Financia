...
describe('friendlyStripeClientError', function() {
  it('prioriza decline_code do erro do Stripe.js', function() {
    const msg = friendlyStripeClientError({ decline_code: 'insufficient_funds', message: 'generic' });
    expect(msg).toMatch(/saldo insuficiente/i);
  });

  it('usa setup_intent.last_setup_error quando disponivel', function() {
    const msg = friendlyStripeClientError({
      code: 'card_declined',
...
describe('readFnErrorMessage', function() {
  it('prioriza data.error quando presente', async function() {
    const msg = await readFnErrorMessage({ error: { message: 'x' } }, { error: 'no_payment_method' });
    expect(msg).toBe('no_payment_method');
  });

  it('sem erro retorna string vazia', async function() {
    const msg = await readFnErrorMessage({ error: null }, null);
    expect(msg).toBe('');
  });

  it('le o corpo JSON do error.context (resposta crua da edge function)', async function() {
    const ctx = { json: function() { return Promise.resolve({ error: 'invalid_plan' }); } };
    const msg = await readFnErrorMessage({ error: { message: 'Edge fail', context: ctx } }, null);
    expect(msg).toBe('invalid_plan');
  });

  it('sem context cai para error.message', async function() {
    const msg = await readFnErrorMessage({ error: { message: 'boom' } }, null);
    expect(msg).toBe('boom');
  });

  it('context.json que rejeita usa error.message como fallback', async function() {
    const ctx = { json: function() { return Promise.reject(new Error('bad')); } };
    const msg = await readFnErrorMessage({ error: { message: 'fallback', context: ctx } }, null);
