...
export default function LogoSchemes({ brandColor, toast }) {
  ...
  var saveScheme = function() {
    ...
    if (!name) { if (toast) toast('Dê um nome para o esquema de cores.', 'warning'); return; }
    var now = Date.now();
    ...
    <div className="flex flex-col gap-5">
      <div className="flex flex-col sm:flex-row gap-5">
        ...
        <div className="flex-1 flex flex-col gap-3">
          ...
          <p className="text-xs" style={{color:'var(--text-muted)'}}>Edite a cor de cada elemento da logo Financia. As alteracoes aparecem em tempo real.</p>
          ...
          <button onClick={resetToOriginal}
            className="text-xs font-medium px-3 py-2 rounded-xl mt-1 transition hover:opacity-80"
      ...
      <div className="border-t pt-4" style={{borderColor:'var(--border)'}}>
        ...
        {schemes.length === 0 && (
          <p className="text-xs text-center py-6" style={{color:'var(--text-muted)'}}>Nenhum esquema salvo ainda. Altere as cores acima e salve.</p>
        )}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <div key="original" className="rounded-xl p-3 flex items-center gap-3 cursor-pointer transition hover:opacity-80"
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px]" style={{color:'var(--text-muted)'}}>Cores padrao do Financia</p>
            </div>
          </div>

          {schemes.map(function(s) {
            return (
              <div key={s.id} className="rounded-xl p-3 flex items-center gap-3 transition hover:opacity-80"
                ...
                <button onClick={function(e) { e.stopPropagation(); deleteScheme(s.id); }}
                  className="w-7 h-7 rounded-lg flex items-center justify-center text-xs hover:opacity-70 flex-shrink-0"
