...
function GoogleBtn({ onClick, loading, label }) {
  return (
    <button type="button" onClick={onClick} disabled={loading} aria-label="Entrar com Google"
      className="w-full flex items-center justify-center gap-3 py-3.5 rounded-xl text-base font-semibold transition hover:bg-gray-50 disabled:opacity-50 relative"
      ...
      {label}
      <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 text-[10px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap" style={{ background: 'rgba(15,157,108,0.12)', color: '#0f9d6c' }}>Recomendado</span>
    </button>
  );
}
