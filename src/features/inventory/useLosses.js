...
export function useLosses(session, enforceLimit, toast) {
  ...
  const addLoss = useCallback(async function(l) {
    if (!session) return false;
    const cnt = await countLimit(ldb, 'losses', session.user.id);
    ...
    const row = {id:l.id, description:l.desc, qty:Number(l.qty), reason:l.reason||null, date:l.date, desc:l.desc, user_id:meta.user_id, registered_by:meta.registered_by, updated_at:meta.updated_at, _synced:meta._synced, _deleted:meta._deleted, _updated_at:meta._updated_at};
  ...
  const editLoss = useCallback(async function(id, u) {
    if (!session) return false;
    ...
    const upd = Object.assign({description:u.desc, qty:Number(u.qty), reason:u.reason||null, date:u.date, desc:u.desc}, updMeta());
  ...
  const deleteLoss = useCallback(async function(id) {
    if (!session) return false;
