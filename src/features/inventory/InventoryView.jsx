...
import { Card, Inp, NumInp, Modal, EditBtn, DelBtn, Badge, Btn, PageHead } from '../../shared/ui/ui.jsx';
import ExportButtons from '../../shared/ui/ExportButtons.jsx';
import { PSearch } from '../../shared/ui/SaleForm.jsx';
...
export default React.memo(function InventoryView({ products, losses, onAddProduct, onEditProduct, onDeleteProduct, onAddLoss, onEditLoss, onDeleteLoss, onAdjustStock, brand, toast, confirm, planInfo, onNav }) {
  ...
  const saveEditL = async function() {
    ...
    if (Number(editL.qty) <= 0) { toast('Quantidade deve ser maior que zero', 'error'); return; }
    dispatch({type:'SET_SAVING', v:true});
    try {
  ...
  const saveStock = async function() {
    if (!sq || !sm) return;
    if (Number(sq) <= 0) { toast('Quantidade deve ser maior que zero', 'error'); return; }
    dispatch({type:'SET_SAVING', v:true});
    try {
    ...
    <div className="flex flex-col gap-5">
        ...
        <Card data-testid="products-list">
          <div className="px-4 py-3 border-b border-gray-50">
            <div className="relative">
              ...
              <input aria-label="Buscar produtos" value={search} onChange={function(e) { dispatch({type:'SET_SEARCH', v:e.target.value}); }} placeholder="Buscar por nome ou categoria..." className="w-full pl-9 pr-3 py-2.5 text-sm border border-gray-200 rounded-xl" style={{background:'var(--bg-input)', color:'var(--text-main)'}}/>
            </div>
          </div>
          {disp.length === 0 ? (
      ...
      {pm && (
        <Modal title="Novo Produto ou Serviço" onClose={function() { dispatch({type:'CLOSE_PM'}); }} onSave={saveProd} saving={saving}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <NumInp label="Preço de venda *" value={pf.price} onChange={function(e) { dispatch({type:'SET_PF', v:{price:e.target.value}}); }} placeholder="0,00"/>
            <NumInp label="Custo" value={pf.cost} onChange={function(e) { dispatch({type:'SET_PF', v:{cost:e.target.value}}); }} placeholder="0,00"/>
          </div>
          <NumInp label="Estoque inicial (em branco para serviços)" decimals={false} maxLen={7} value={pf.stock} onChange={function(e) { dispatch({type:'SET_PF', v:{stock:e.target.value}}); }} placeholder="Ex: 50"/>
        </Modal>
      )}
      {editP && (
        <Modal title="Editar Produto" onClose={function() { dispatch({type:'SET_EDIT_P', v:null}); }} onSave={saveEditP} saving={saving} saveLabel="Salvar alterações">
          ...
          <div className="grid grid-cols-2 gap-3">
            <NumInp label="Preço de venda *" value={editP.price} onChange={function(e) { dispatch({type:'PATCH_EDIT_P', v:{price:e.target.value}}); }}/>
            <NumInp label="Custo" value={editP.cost} onChange={function(e) { dispatch({type:'PATCH_EDIT_P', v:{cost:e.target.value}}); }} placeholder="Opcional"/>
          </div>
          <NumInp label="Estoque atual" decimals={false} maxLen={7} value={editP.stock} onChange={function(e) { dispatch({type:'PATCH_EDIT_P', v:{stock:e.target.value}}); }} placeholder="Em branco para serviços"/>
        </Modal>
      )}
      {lm && (
      ...
      {sm && sp && (
        <Modal title={'Repor estoque: ' + sp.name} onClose={function() { dispatch({type:'SET_SM', v:null}); }} onSave={saveStock} saving={saving} saveLabel="Adicionar">
          ...
          <NumInp label="Quantidade a adicionar" decimals={false} maxLen={6} value={sq} onChange={function(e) { dispatch({type:'SET_SQ', v:e.target.value}); }} placeholder="Ex: 10"/>
