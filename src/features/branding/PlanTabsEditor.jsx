    ...
    <div className="flex flex-col gap-4">
      <div className="flex border-b gap-1" style={{borderColor:'var(--border, #e2e8f0)'}} role="tablist" aria-label="Planos">
        ...
        <button onClick={onCopyDocs}
          className="text-xs font-semibold px-3 py-1.5 rounded-lg transition hover:opacity-80 flex items-center gap-1.5 min-h-[var(--touch-target-min)]"
...
function ColorInput({ label, value, onChange, desc }) {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="flex items-center gap-2">
        <input type="color" id={inputId} value={value || '#000000'} onChange={e => onChange(e.target.value)} className="min-w-[var(--touch-target-min)] min-h-[var(--touch-target-min)] w-8 h-8 rounded-lg cursor-pointer border-0 p-0.5 flex-shrink-0" aria-label={label} />
