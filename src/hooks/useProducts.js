...
export function useProducts(session, enforceLimit, toast) {
  ...
  var addProduct = async function(p) {
    var cnt = await countLimit(ldb, 'products', session.user.id);
    ...
    var row = {id:p.id, name:p.name, category:p.category||null, price:Number(p.price), cost:Number(p.cost)||0, stock:stock, user_id:meta.user_id, registered_by:meta.registered_by, updated_at:meta.updated_at, _synced:meta._synced, _deleted:meta._deleted, _updated_at:meta._updated_at};
  ...
  var editProduct = async function(id, u) {
    ...
    var upd = Object.assign({name:u.name, category:u.category||null, price:Number(u.price), cost:Number(u.cost)||0, stock:(u.stock!==''&&u.stock!=null)?Number(u.stock):null}, updMeta());
  ...
  var adjustStock = async function(id, delta) {
    ...
    var upd = Object.assign({stock:ns}, updMeta());
    if (!await dexieUpdate(ldb, 'products', id, upd, toast)) return false;
    setProducts(function(p) { return applyEdit(p, id, upd); });
