...
import { useSwipeActions } from '../../shared/hooks/useSwipeActions.js';
...
export var TransactionCard = React.memo(function TransactionCard({ 
  ...
  var swipeActions = showActions ? [
    { label: 'Duplicar', dir: 'left', color: 'var(--success)', icon: 'M12 4v4m0 0l-3-3m3 3l3-3M7 12h10m-7 4h7m-7 4h7', onAction: function() { if (onDuplicate) onDuplicate(transaction); } },
    { label: 'Excluir',  dir: 'left', color: 'var(--danger)',  icon: 'M6 18L18 6M6 6l12 12', onAction: function() { if (onConfirm && onDelete) onConfirm('Excluir este registro?', function() { onDelete(transaction.id); }); } },
    { label: 'Editar',   dir: 'right', color: 'var(--info)',  icon: 'M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7', onAction: function() { onEdit(transaction); } },
  ] : [];
  var swipe = useSwipeActions({ actions: swipeActions, threshold: 60 });
  ...
  var swipeOffset = swipe.isSwiping || swipe.offset !== 0 ? swipe.offset : 0;
  var swipeProgress = Math.abs(swipeOffset) > 60;

  return (
    <div
      ref={swipe.elementRef}
      data-testid={'tx-item-' + transaction.id}
      role="listitem"
      aria-setsize={totalCount}
      aria-posinset={index + 1}
      className="relative px-4 py-3.5 border-b border-gray-50 last:border-0 transition-colors overflow-hidden"
      style={{ background: swipe.isSwiping ? 'var(--bg-card)' : undefined }}
    >
      {showActions && (
        <div className="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none" style={{ opacity: swipeProgress ? 1 : 0, transition: 'opacity 150ms' }}>
          <div className="flex gap-1">
            <span className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: 'var(--success)', opacity: swipeOffset < 0 ? 0.2 : 0.08 }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round"><path d="M12 4v4m0 0l-3-3m3 3l3-3M7 12h10m-7 4h7m-7 4h7"/></svg>
            </span>
            <span className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: 'var(--danger)', opacity: swipeOffset < 0 ? 0.2 : 0.08 }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round"><path d="M3 6h18M19 6l-2 2H7L5 6m-2 0v14a2 2 0 002 2h10a2 2 0 012-2v-4M9.5 11h5"/></svg>
            </span>
          </div>
        </div>
      )}
